...
#echo "seconds / day = $seconds_per_day"
if [ ! -z $to ] && [ ! -z $from ]; then
  echo $(( ($( date -d "$to 12:00" +%s ) - $( date -d "$from 12:00" +%s )) / $seconds_per_day ))
fi

if [ -z $to ] && [ -z $delta ]; then
  echo $(( ($( date -d "`date +%Y-%m-%d` 12:00" +%s ) - $( date -d "$from 12:00" +%s )) / $seconds_per_day ))
fi

if [ -z $from ] && [ -z $delta ]; then
  echo $(( ($( date -d "$to 12:00" +%s ) - $( date -d "`date +%Y-%m-%d` 12:00" +%s )) / $seconds_per_day ))
fi
